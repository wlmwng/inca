...
class missing_text_americanrenaissance(Processer):
    ...
    def process(self, document_field):
        ...
        text_is_generic = any([text == "AmRen is America's foremost white advocacy publication. To keep up with our latest, follow us on Telegram Gab , and BitChute",
                               text == "These content recommendations are provided by AdStyle and may be paid by the advertiser whose ad you clicked on. Ad.Style recognizes interest based on how you and other visitors interact with content suggestions."])
...
class missing_text_dailystormer(Processer):
    ...
    def process(self, document_field):
        ...
        # conditions for marking indicator as true
        text_is_empty = True if len(text)==0 else False
        text_is_generic = any([text == "We here at the Daily Stormer are opposed to violence. We seek revolution through the education of the masses. When the information is available to the people, systemic change will be inevitable and unavoidable.\nAnyone suggesting or promoting violence in the comments section will be immediately banned, permanently."])
...
class missing_text_foxnews(Processer):
    ...
    def process(self, document_field):
        ...
        # conditions for marking indicator as true
        text_is_empty = True if len(text)==0 else False
        text_is_generic = any([text == "By using this site, you agree to our Privacy Policy and our Terms of Use",
...
class missing_text_gatewaypundit(Processer):
    ...
    def process(self, document_field):
        ...
        # conditions for marking indicator as true
        text_is_empty = True if len(text)==0 else False
        text_is_generic = any([text == "The page you are looking for no longer exists. Perhaps you can return back to the site's homepage and see if you can find what you are looking for. Or, you can try finding it with the information below."])
...
class missing_text_infowars(Processer):
    ...
    def process(self, document_field):
        ...
        # conditions for marking indicator as true
        text_is_empty = True if len(text)==0 else False
        text_is_generic = any([text == "Keep up to date with our latest:\nHave an important tip? Let us know. Email us here."])
...
class missing_text_oneamericanews(Processer):
    ...
    def process(self, document_field):
        ...
        # conditions for marking indicator as true
        text_is_empty = True if len(text)==0 else False
        text_is_generic = any([text == "It seems we can’t find what you’re looking for. Perhaps searching can help.\nSearch for:",
        ...
        # evaluate the conditions
        missing_ind = any([text_is_empty, text_is_generic, text_is_likely_unrelated_preview])
